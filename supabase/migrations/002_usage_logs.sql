-- Usage logging table for all API requests
-- Referenced by: supabase_auth.py (rate limiting), hosted_budget.py (spend tracking),
-- analyticsService.ts (dashboard), supabase_db.py (logging)
...
BEGIN
  INSERT INTO usage_logs (
    request_id, user_id, model_name, provider,
    tokens_in, tokens_out, cost, route,
    prompt, response, latency_ms, cluster_id, metadata
  ) VALUES (
    p_request_id, p_user_id, p_model_name, p_provider,
    p_tokens_in, p_tokens_out, p_cost, p_route,
    p_prompt, p_response, p_latency_ms, p_cluster_id, p_metadata
  ) RETURNING id INTO v_id;

  -- Update monthly cost on profile
  UPDATE profiles
  SET cost_this_month = COALESCE(cost_this_month, 0) + p_cost
  WHERE id = p_user_id;
...
DECLARE
  ...
  v_used NUMERIC;
BEGIN
  SELECT budget_limit, COALESCE(cost_this_month, 0)
  INTO v_limit, v_used
  FROM profiles
  WHERE id = p_user_id
  ...
  -- Reserve the budget
  UPDATE profiles
  SET cost_this_month = v_used + p_estimated_cost
  WHERE id = p_user_id;
...
-- Presets table (for API key routing configurations)
...
CREATE POLICY presets_user_policy ON presets
  FOR ALL USING (auth.uid() = user_id);

-- Provider keys table (for BYOK)
